...
const convertErrorToApiError = (context: Context, error: unknown) => {
    if (error instanceof DomainError) {
        context.services.application.logging.log(`Domain error handled. Error: ${JSON.stringify(error)}`);
        return ApiError.fromDomainError(error);
    }
    else if (error instanceof ApplicationError) {
        context.services.application.logging.log(`Application error handled. Error: ${JSON.stringify(error)}`);
        return ApiError.fromApplicationError(error);
    }
    else if (error instanceof InfrastructureError) {
        context.services.application.logging.error(`Infrastructure error handled. Error: ${JSON.stringify(error)}`);
        return ApiError.fromInfrastructureError(error);
    }
    else {
        context.services.application.logging.error(`Unknown error handled. Error: ${JSON.stringify(error)}`);
        return ApiError.fromUnknownError();
    }
};

const apiErrorConverter = (context: Context) =>
